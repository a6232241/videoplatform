...
export default {
  ...
  signup () {
    ...
    let pwd = $('.register-form').children('input[name="password"]')
    let repeatPwd = $('.register-form').children('input[name="re-password"]')
    let sendBtn = $('.register-form').children('button')

    name.change((e) => {
      verifyAccount(e)
    })
    verifyPassword(pwd, repeatPwd)
    ...
    let checkPwd = false
    // 確認密碼
    function verifyPassword (pwd, againPwd) {
      againPwd.keyup(() => {
    ...
    // 傳遞資料
    function submitAccount (sendBtn, form) {

      sendBtn.on('click', (e) => {
        e.preventDefault()
        if (noRepeat == "true" && checkPwd) {
